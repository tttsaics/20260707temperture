...
Thermal::Thermal() : rawFrame(nullptr), frame(nullptr), fps(0), baudrate(0), ta(0.0f), maxTemp(0.0f), minTemp(0.0f), foreheadTemp(0.0f), status(ThermalStatus::NOT_INITIALIZED) {
    try {
        //動態配置原始資料的陣列空間 (大小為 10248 bytes)
        rawFrame = new uint8_t[RAW_FRAME_SIZE];
        //動態配置浮點數陣列空間，用來存放解析後的像素溫度 (80*62 = 4960)
        frame = new float[FRAME_SIZE];
    } catch (...) {
        //異常安全保護：若分配失敗則釋放已分配資源，避免記憶體洩漏
        delete[] rawFrame;
        delete[] frame;
        throw;
    }
    
    // 以二進位 (binary) 模式開啟感測器資料檔 data.bin
...
Thermal::~Thermal() {
    //解構時，先檢查 rawFrame 指標是否非空
    if (rawFrame) {
        //安全地釋放 rawFrame 的陣列記憶體
        delete[] rawFrame;
    }
    //檢查 frame 指標是否非空
    if (frame) {
...
bool Thermal::readFrame() {
    ...
    //呼叫 read 將 RAW_FRAME_SIZE (10248) 個位元組讀進 rawFrame 陣列
    dataFile.read(reinterpret_cast<char*>(rawFrame), RAW_FRAME_SIZE);
    
    //使用 gcount 檢查這一次實際讀取到的位元組數是否等於一個完整的 frame
    ...
    //成功讀取到 1 個完整的 frame，回傳 true
    status = ThermalStatus::SUCCESS;
    return true;
}

bool Thermal::parserFrame() {
    //如果 rawFrame 尚未配置記憶體，則避免發生執行錯誤並回傳 false
    if (!rawFrame) {
        status = ThermalStatus::NOT_INITIALIZED;
        return false;
    }

    //檢查 frame 檔頭簽章是否為 "   #2808GFRA"
    ...
    //定義像素資料的起始位移值為 byte 328
    int pixelOffset = 328;
    //利用迴圈逐一解析 80x62 (4960) 個像素的資料 (為階段二預先準備)
    for (std::size_t i = 0; i < FRAME_SIZE; i++) {
        //配合 ta 與 maxTemp 採用 Big-Endian (大端序) 進行一致性解析
        size_t offset = pixelOffset + i * 2;
        //防禦性邊界檢查：防止 rawFrame 陣列越界訪問
        if (offset + 1 >= RAW_FRAME_SIZE) {
            return false;
        }
        uint16_t rawVal = (rawFrame[offset] << 8) | rawFrame[offset + 1];
        //將像素數值除以 10 並減去 273.15 轉換為攝氏溫度存入 frame
...
bool Thermal::saveFrame(std::string filename) {
    //階段二實作：直接輸出為 640x496 (80x62 放大 8 倍) 的 24-bit BMP 灰階圖檔
    int width = 80;
    int height = 62;
    int scale = 8;
    ...
    for (int y = outHeight - 1; y >= 0; --y) {
        ...
        for (int x = 0; x < outWidth; ++x) {
            ...
            // BMP 使用 BGR 位元組順序，灰階時 B、G、R 數值皆相同
            row[x * 3] = gray;
            row[x * 3 + 1] = gray;
            row[x * 3 + 2] = gray;
        }
